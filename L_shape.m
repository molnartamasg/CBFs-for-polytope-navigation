...
% obstacle geometry
par.om=0;                       % angular velocity of obstacle  % 0, 0.5
par.wc=[4;4];                   % center of obstacle
par.dw=[2;1].*[ 1, 1,0,0,-1,-1; % vertices of obstacle relative to center
               -1, 0,0,1, 1,-1];
Nw=size(par.dw,2);
...
par.b=0.7;          % buffer
                    % b=5*log(2)/kappa (2 barriers in min and 5 convex sets in max)
...
function [h,dhdpsi] = smaxmin(psi,par)
    ...
    Phi=1./(Psi*par.A);                 % combine convex corners of obstacle
    sumPhi=sum(Phi);                    % combine convex sets
    h=(log(sumPhi)-par.b)/par.kappa;    % smoothen with log and buffer
    dhdpsi=(Phi.^2*par.A.').*Psi/sumPhi;% calculate gradient
end
